...
def main():
    ...
    # STEP 10: Find Best Match for Each Task and Overall
    print("\n" + "="*80)
    print("BEST OBJECT FOR EACH TASK (Based on Loaded Matrix)")
    print("="*80)
    
    for t_idx, task in enumerate(tasks):
        task_scores = loaded_matrix[t_idx]
        best_obj_idx = np.argmax(task_scores)
        best_obj = objects[best_obj_idx]
        best_score = task_scores[best_obj_idx]
        print(f"Task: {task:<25} -> Best Object: {best_obj:<15} (Score: {best_score:.4f})")

    print("\n" + "="*80)
    print("OVERALL HIGHEST RELEVANT VALUE")
    print("="*80)
    
    max_idx = np.unravel_index(np.argmax(loaded_matrix, axis=None), loaded_matrix.shape)
    best_overall_task = tasks[max_idx[0]]
    best_overall_obj = objects[max_idx[1]]
    best_overall_val = loaded_matrix[max_idx]
